...
average_survival_rate <-  traffic_collision %>%
    ...
    summarise(survival_rate = percent(mean((PERSONCOUNT - FATALITIES)
                                           / PERSONCOUNT, na.rm = TRUE),
                                      accuracy = 0.01)) %>%
    pull(survival_rate)
...
rate_injuries_speeding <-  dataset %>%
    filter(SPEEDING == "Y") %>%
    filter(PERSONCOUNT != 0) %>%
    summarise(average_injury_rate = percent(
      mean(INJURIES / PERSONCOUNT, na.rm = TRUE), accuracy = 0.01)) %>% 
